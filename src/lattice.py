...
class Lattice:
    def __init__(self, logger: Logger, lattice_ip: str, bearer_token: str):
        ...
        self.lattice_ip = (
            lattice_ip
            if lattice_ip.startswith("https://")
            else f"https://{lattice_ip}"
        )
    ...
    async def get_entity(self, entity_id) -> Optional[Dict[str, Any]]:
        ...
        try:
            async with aiohttp.ClientSession(
                headers=self.generated_metadata
            ) as session:
                async with session.get(
                    f"{self.lattice_ip}/api/v1/entities/{entity_id}"
                ) as response:
        ...
        try:
            async with aiohttp.ClientSession(
                ...
                async with session.put(
                    f"{self.lattice_ip}/api/v1/entities", json=entity
                ) as response:
                    response.raise_for_status()
                    entity_data = await response.json()
                    return entity_data
